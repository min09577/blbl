...
object DlnaHelper {
    ...
    data class DlnaDevice(
        ...
        val host: String
            get() = try { java.net.URL(location).host } catch (_: Exception) { "" }
    }

    /**
     * SSDP M-SEARCH: 搜索局域网内所�?MediaRenderer 设备
     */
    suspend fun discoverDevices(timeoutMs: Int = SEARCH_TIMEOUT_MS): List<DlnaDevice> = withContext(Dispatchers.IO) {
     ...
     */
    private suspend fun fetchDeviceInfo(device: DlnaDevice): DlnaDevice = withContext(Dispatchers.IO) {
    ...
    private fun findAvTransportControlUrl(xml: String): String? {
        ...
        val servicePattern = Regex(
            "<service>\\s*" +
                "<serviceType>urn:schemas-upnp-org:service:AVTransport:1</serviceType>.*?" +
                "<controlURL>(.*?)</controlURL>",
            RegexOption.DOT_MATCHES_ALL
        )
        return servicePattern.find(xml)?.groupValues?.get(1)?.trim()
    }

    /**
     * 发�?SetAVTransportURI + Play 指令�?DLNA 设备
     */
    suspend fun castUrl(device: DlnaDevice, videoUrl: String, title: String = ""): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            val controlFullUrl = if (device.controlUrl.startsWith("http")) {
                device.controlUrl
            } else {
     ...
     * 发�?Stop 指令
     */
    suspend fun stopCast(device: DlnaDevice): Boolean = withContext(Dispatchers.IO) {
        try {
            val controlFullUrl = if (device.controlUrl.startsWith("http")) {
                device.controlUrl
            } else {
    ...
    private fun sendSoapAction(url: String, soapAction: String, body: String) {
        val client = okHttpClient
    ...
    private fun buildSetAvTransportUriSoap(uri: String, title: String): String = """
        ...
        <s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/" s:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/">
            <s:Body>
                <u:SetAVTransportURI xmlns:u="urn:schemas-upnp-org:service:AVTransport:1">
                    ...
                    <CurrentURIMetaData>&lt;DIDL-Lite xmlns="urn:schemas-upnp-org:metadata-1-0/DIDL-Lite/" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:upnp="urn:schemas-upnp-org:metadata-1-0/upnp/"&gt;&lt;item id="0" parentID="-1" restricted="1"&gt;&lt;dc:title&gt;${escapeXml(title)}&lt;/dc:title&gt;&lt;res protocolInfo="http-get:*:video/mp4:*"&gt;${escapeXml(uri)}&lt;/res&gt;&lt;upnp:class&gt;object.item.videoItem&lt;/upnp:class&gt;&lt;/item&gt;&lt;/DIDL-Lite&gt;</CurrentURIMetaData>
                </u:SetAVTransportURI>
            </s:Body>
        </s:Envelope>
    """.trimIndent()
    ...
    suspend fun setAVTransportURI(device: DlnaDevice, url: String, title: String = "") {
    ...
    // v5.1: 获取播放进度
    data class PositionInfo(val currentPos: Long, val duration: Long)
    ...
    private fun buildControlUrl(device: DlnaDevice): String {
        return if (device.controlUrl.startsWith("http")) {
            device.controlUrl
        } else {
            "http://${device.host}:80${if (device.controlUrl.startsWith("/")) "" else "/"}${device.controlUrl}"
        }
