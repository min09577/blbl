...
internal fun PlayerActivity.initNetworkSpeedIndicator() {
    ...
    networkSpeedJob = lifecycleScope.launch {
        while (isActive) {
            val bps = debug.downloadSpeedBps
            tvSpeed.text = if (bps > 0) {
...
internal fun PlayerActivity.toggleCurrentPositionBookmark() {
    ...
    if (existing != null) {
        ...
        android.widget.Toast.makeText(this, "已删除书签", android.widget.Toast.LENGTH_SHORT).show()
    } else {
        BiliClient.prefs.addVideoBookmark(bvid, position)
        android.widget.Toast.makeText(this, "已添加书签", android.widget.Toast.LENGTH_SHORT).show()
    }
    updateBookmarkIndicator()
}
...
internal fun PlayerActivity.showBookmarksDialog() {
    ...
    android.app.AlertDialog.Builder(this)
        ...
        .setNeutralButton("清空") { _, _ ->
            ...
            updateBookmarkIndicator()
            android.widget.Toast.makeText(this, "已清空书签", android.widget.Toast.LENGTH_SHORT).show()
        }
