...
internal fun PlayerActivity.showVideoColorAdjustmentDialog() {
    ...
    // 亮度
    val brightnessLabel = TextView(this).apply { text = "亮度: $brightness"; textSize = 14f }
    val brightnessSlider = SeekBar(this).apply { max = 200; progress = brightness + 100 }
    layout.addView(brightnessLabel)
    layout.addView(brightnessSlider)
    
    // 对比度
    val contrastLabel = TextView(this).apply { text = "对比度: $contrast"; textSize = 14f; setPadding(0, 16, 0, 0) }
    val contrastSlider = SeekBar(this).apply { max = 200; progress = contrast + 100 }
    layout.addView(contrastLabel)
    layout.addView(contrastSlider)
    
    // 饱和度
    val saturationLabel = TextView(this).apply { text = "饱和度: $saturation"; textSize = 14f; setPadding(0, 16, 0, 0) }
    val saturationSlider = SeekBar(this).apply { max = 200; progress = saturation + 100 }
