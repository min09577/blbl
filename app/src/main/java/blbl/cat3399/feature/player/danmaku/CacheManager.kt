...
internal data class CacheStyle(
    ...
    val strokeColor: Int = 0x000000, // v12.23: 描边颜色
...
internal class CacheManager(
    ...
    private fun buildCache(req: CacheRequest) {
        ...
        // v12.23: 使用可配置的描边颜色
        val strokeRgb = style.strokeColor and 0xFFFFFF
        stroke.color = (0xCC shl 24) or strokeRgb
        fill.color = (0xFF shl 24) or rgb

        // v6.4: 弹幕关键词高亮
        val highlightKeywords = blbl.cat3399.core.net.BiliClient.prefs.danmakuHighlightKeywords
        if (highlightKeywords.isNotEmpty()) {
            val text = danmaku.text
            val matched = highlightKeywords.any { keyword -> text.contains(keyword, ignoreCase = true) }
            if (matched) {
                val highlightColor = blbl.cat3399.core.net.BiliClient.prefs.danmakuHighlightColor and 0xFFFFFF
                fill.color = (0xFF shl 24) or highlightColor
                stroke.color = (0xCC shl 24) or strokeRgb // v12.23: 使用可配置的描边颜色
            }
        }
