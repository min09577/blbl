...
private fun PlayerActivity.populateQuickSettings(panel: LinearLayout) {
    ...
    addToggleRow(panel, "循环播放", loopOn) { enabled ->
        ...
        android.widget.Toast.makeText(this, if (enabled) "循环播放：开" else "循环播放：关", android.widget.Toast.LENGTH_SHORT).show()
    }
    ...
    addToggleRow(panel, "护眼模式", eyeOn) { enabled ->
        ...
        android.widget.Toast.makeText(this, if (enabled) "护眼模式：开" else "护眼模式：关", android.widget.Toast.LENGTH_SHORT).show()
    }
    ...
    for (i in speeds.indices) {
        row.addView(TextView(this).apply {
            ...
            setOnClickListener {
                ...
                android.widget.Toast.makeText(this@addSpeedRow, "播放速度: ${labels[i]}", android.widget.Toast.LENGTH_SHORT).show()
            }
        })
    }
    panel.addView(row)
    ...
    for (i in qns.indices) {
        row.addView(TextView(this).apply {
            ...
            setOnClickListener {
                ...
                hideQuickSettingsPanel()
                android.widget.Toast.makeText(this@addQualityRow, "画质: ${names[i]}（下次播放生效）", android.widget.Toast.LENGTH_SHORT).show()
            }
        })
    }
    panel.addView(row)
