...
private fun autoNextModeAllowsNext(mode: String): Boolean {
    return when (mode) {
        ...
        AppPrefs.PLAYER_PLAYBACK_MODE_RANDOM,
        -> true
...
private fun PlayerActivity.shouldShowAutoNextHintFallback(mode: String): Boolean {
    ...
    return when (mode) {
        ...
        AppPrefs.PLAYER_PLAYBACK_MODE_RANDOM -> true
        else -> false
    }
}
...
private fun PlayerActivity.resolveRandomAutoNextTarget(): AutoNextTarget? {
    ...
    val allItems = mutableListOf<AutoNextTarget>()
    val pageTarget = resolvePageAutoNextTarget()
    if (pageTarget != null) allItems.add(pageTarget)
    val partsTarget = resolvePartsAutoNextTarget()
    if (partsTarget != null) allItems.add(partsTarget)
    val recommendTarget = resolveRecommendedAutoNextTarget(preloadRecommendation = false)
    if (recommendTarget != null) allItems.add(recommendTarget)
    return if (allItems.isNotEmpty()) allItems.random() else null
}
