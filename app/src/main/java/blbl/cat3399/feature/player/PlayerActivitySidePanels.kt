...
import blbl.cat3399.core.api.VideoApi
...
internal fun PlayerActivity.initSidePanels() {
    ...
    binding.chipCommentSortReply.setOnFocusChangeListener { _, hasFocus ->
        if (hasFocus) switchCommentSortOnFocus(COMMENT_SORT_REPLY)
    }
    updateCommentSortUi()
    ...
    run {
        val adapter =
            PlayerCommentsAdapter(
                ...
                onLongClick = { item ->
                    ...
                    if (!hasPictures) {
                        if (item.message.isNotBlank()) {
                            val clip = android.content.ClipData.newPlainText("comment", item.message)
                            val cm = getSystemService(android.content.Context.CLIPBOARD_SERVICE) as android.content.ClipboardManager
                            cm.setPrimaryClip(clip)
                            AppToast.show(this, "已复制评论")
                        }
                ...
                onReply = { item ->
                    if (!isCommentsPanelVisible()) return@PlayerCommentsAdapter
                    showCommentReplyDialog(item)
                },
                onLike = { rpid, like ->
                    val aid = currentAid ?: return@PlayerCommentsAdapter
                    val ctx = this@run
                    lifecycleScope.launch {
                        try {
                            VideoApi.commentLike(oid = aid, type = COMMENT_TYPE_VIDEO, rpid = rpid, like = like)
                            AppToast.show(ctx, if (like) "已点赞" else "已取消点赞")
                        } catch (e: Exception) {
                            AppToast.show(ctx, "操作失败：${e.message}")
                        }
                    }
                },
            )
...
// v12.0: 评论回复功能 - 显示回复对话框
internal fun PlayerActivity.showCommentReplyDialog(replyItem: PlayerCommentsAdapter.Item) {
    val ctx = this
    val oid = currentAid
    val type = COMMENT_TYPE_VIDEO
    if (oid == null || oid <= 0L || type <= 0) {
        AppToast.show(ctx, "无法获取视频信息")
        return
    }
    // 检查登录状态
    ...
    btnSend.setOnClickListener {
        ...
        if (message.isBlank()) {
            AppToast.show(ctx, "回复内容不能为空")
            return@setOnClickListener
        }
        btnSend.isEnabled = false
        lifecycleScope.launch {
            runCatching {
                VideoApi.commentReply(
                    oid = oid,
                    type = type,
                    root = root,
                    parent = parent,
                ...
                AppToast.show(ctx, "回复失败: ${it.message}")
            }
        }
    }
    dialog.show()
...
// v12.6: 评论楼层跳转对话框
private fun PlayerActivity.showCommentFloorJumpDialog() {
    ...
    android.app.AlertDialog.Builder(this)
        ...
        .setPositiveButton("跳转") { _, _ ->
            val floorText = input.text.toString().trim()
            val floor = floorText.toIntOrNull()
            if (floor == null || floor <= 0) {
                AppToast.show(this, "请输入有效的楼层号")
                return@setPositiveButton
            }
            val targetPos = commentsItems.indexOfFirst { it.floor == floor }
            if (targetPos < 0) {
                AppToast.show(this, "未找到第${floor}楼")
                return@setPositiveButton
            }
...
private fun PlayerActivity.focusSelectedCommentSortChip() {
    val target =
        when (commentSort) {
            ...
            COMMENT_SORT_REPLY -> binding.chipCommentSortReply
...
internal fun PlayerActivity.applyCommentSort(sort: Int) {
    ...
    commentSort = sort
    BiliClient.prefs.commentSort = sort // v9.2: 记住排序偏好
    updateCommentSortUi()
...
internal fun PlayerActivity.updateCommentSortUi() {
    ...
    val replySelected = commentSort == COMMENT_SORT_REPLY

    binding.chipCommentSortHot.isSelected = hotSelected
    binding.chipCommentSortNew.isSelected = newSelected
    binding.chipCommentSortReply.isSelected = replySelected
    ...
    binding.chipCommentSortReply.setTextColor(if (replySelected) selected else unselected)
}
...
private fun parseReplyItem(
    ...
    // v5.2: 评论楼层号
    val floor = obj.optInt("floor", 0).coerceAtLeast(0)
    // v7.7: 用户等级
    val userLevel = member.optJSONObject("level_info")?.optInt("current_level", 0) ?: 0
    // v7.8: 用户签名
    val userSign = member.optString("sign", "").trim()
    // v9.7: IP属地
    val ipLocation = obj.optJSONObject("reply_control")?.optString("location", "")?.trim() ?: ""
