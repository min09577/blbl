        ...
        lifecycleScope.launch {
            try {
                ...
                AppPopup.singleChoice(
                    ...
                    // v4.4: handle create new folder
                    if (index == folders.size) {
                        showCreateFavFolderDialog(requestAid, initial)
...
private fun PlayerActivity.showCreateFavFolderDialog(
    ...
    android.app.AlertDialog
        ...
        .setPositiveButton("创建") { _, _ ->
            ...
            lifecycleScope.launch {
                try {
                    val newFolderId = BiliApi.favFolderCreate(title)
                    if (newFolderId > 0) {
                        // Auto-add current video to new folder
                        BiliApi.favResourceDeal(
                            rid = rid,
                            addMediaIds = listOf(newFolderId),
                            delMediaIds = emptyList(),
                        )
                        AppToast.show(this@showCreateFavFolderDialog, "收藏夹“$title”已创建并收藏")
                    }
                } catch (t: Throwable) {
...
// v12.5: 复制视频链接到剪贴板
internal fun PlayerActivity.copyVideoLink() {
    ...
    clipboard.setPrimaryClip(ClipData.newPlainText("bilibili_video_url", url))
