...
internal class IjkPlayerEngine(
    ...
    /**
     * 直播优化参数 — 精简版，只保留必要项
     * 原版 IjkPlayer 用默认参数（无任何直播特殊配置），稳定性好但延迟高。
     * 这里仅添加 liveplay 标志 + 关闭精确seek，其余保持原版默认值。
     */
    private fun applyLiveOptions(p: IjkMediaPlayer) {
        // Enable liveplay mode — tells ijk to treat the stream as live (reduces latency slightly).
        runCatching { p.setOption(IjkMediaPlayer.OPT_CATEGORY_PLAYER, "liveplay", 1L) }
        // Disable accurate seek for live streams (not applicable, saves CPU).
        runCatching { p.setOption(IjkMediaPlayer.OPT_CATEGORY_PLAYER, "enable-accurate-seek", 0L) }
        // Apply FPS limit if set (0 = native/no limit).
        val fps = liveFps
        if (fps > 0) {
            runCatching { p.setOption(IjkMediaPlayer.OPT_CATEGORY_PLAYER, "max-fps", fps.toLong()) }
            if (BuildConfig.DEBUG) {
                AppLog.i("IjkEngine", "live max-fps=$fps")
            }
        }
        if (BuildConfig.DEBUG) {
            AppLog.i("IjkEngine", "live options applied (fps=$fps)")
        }
    }
    ...
    private companion object {
        ...
        private const val LIVE_MAX_RECONNECT_ATTEMPTS: Int = 5
        private const val LIVE_RECONNECT_BASE_DELAY_MS: Long = 1000L
        private const val LIVE_RECONNECT_MAX_DELAY_MS: Long = 8000L
    }
}
