...
class PlayerActivity : BaseActivity() {
    ...
    internal var networkSpeedJob: kotlinx.coroutines.Job? = null
    internal var watchTimeJob: kotlinx.coroutines.Job? = null
    ...
    internal var commentSort: Int = BiliClient.prefs.commentSort
    ...
    private fun requestExitCleanup(reason: String) {
        ...
        // v12.6: 更新播放统计
        try {
            val watchStatsPrefs = getSharedPreferences("watch_stats", android.content.Context.MODE_PRIVATE)
            val currentPos = player?.currentPosition ?: 0L
            if (currentPos > 0) {
                val prev = watchStatsPrefs.getLong("total_watch_time", 0L)
                val count = watchStatsPrefs.getInt("video_count", 0)
                watchStatsPrefs.edit().apply {
                    putLong("total_watch_time", prev + currentPos)
                    putInt("video_count", count + 1)
                    putLong("last_watch_time", System.currentTimeMillis())
                    if (!watchStatsPrefs.contains("install_time")) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (prefs.volumeMemoryEnabled && prefs.lastVideoVolume >= 0) {
            try {
                ...
                val targetVolume = (prefs.lastVideoVolume.toInt() * maxVolume / 100)
                audioManager.setStreamVolume(AudioManager.STREAM_MUSIC, targetVolume.coerceIn(0, maxVolume), 0)
            } catch (e: Exception) {
                // Ignore errors
            }
        }
        ...
        // v6.8: 恢复该视频上次的播放速度
        val savedSpeed = BiliClient.prefs.getVideoSpeed(currentBvid)
        if (!savedSpeed.isNaN() && savedSpeed != session.playbackSpeed) {
            engine.setPlaybackSpeed(savedSpeed)
            session = session.copy(playbackSpeed = savedSpeed)
        }
        // v13.16: 播放位置记忆 - 恢复上次播放位置
        if (prefs.playbackPositionMemoryEnabled) {
            val savedPosition = BiliClient.prefs.getVideoPosition(currentBvid, currentCid)
            val duration = engine.duration
            // Only seek if position is valid (not near the end, and less than duration)
            if (savedPosition > 0 && duration > 0 && savedPosition < duration - 5000) {
        ...
        engine.addListener(
            object : BlblPlayerEngine.Listener {
                ...
                override fun onPlaybackStateChanged(playbackState: Int) {
                    ...
                    // v6.2: 播放开始时显示画质信息
                    if (playbackState == Player.STATE_READY && !qualityInfoShown) {
                    ...
                    if (playbackState == Player.STATE_READY && engine.playWhenReady) {
                        val skipIntro = BiliClient.prefs.skipIntroSeconds
                        if (skipIntro > 0 && engine.currentPosition in 0 until (skipIntro * 1000L)) {
                            engine.seekTo(skipIntro * 1000L)
                        }
                    }
        ...
        applyVideoRotation(session.videoRotation) // v12.14: 应用视频旋转
        applyVideoMirror(session.videoMirror) // v12.15: 应用视频镜像
        // v12.21: 启动播放时长提醒
        val reminderMinutes = BiliClient.prefs.playbackTimeReminderMinutes
        if (reminderMinutes > 0) startPlaybackTimeReminder(reminderMinutes)
    ...
    override fun onDestroy() {
        ...
        unregisterHeadphoneDisconnectReceiver() // v7.1
    ...
    private fun initControls(engine: BlblPlayerEngine) {
        ...
        initCastButton(binding) // v4.13: DLNA投屏
        registerHeadphoneDisconnectReceiver() // v7.1: 耳机断开自动暂停
        smartQualityController.start()
        ...
        // v13.0: 长按设置按钮呼出快捷操作面板
        binding.btnAdvanced.setOnLongClickListener {
            showQuickPanel()
            true
        }
...
//            if (debug) AppLog.d("Player", "danmaku prefetch skipped: interval")
                ...
                try {
                    ...
                    withContext(NonCancellable + Dispatchers.Main) {
                        ...
                        for ((seg, items) in loadedSegmentItems) {
                            ...
                            binding.danmakuView.appendDanmakus(items, maxItems = BiliClient.prefs.danmakuMaxCount, alreadySorted = true)
                            appendedAny = true
                        }
    ...
    // v6.5: 视频弹幕发送对话框
    private fun showSendVideoDanmakuDialog() {
        val cid = currentCid
