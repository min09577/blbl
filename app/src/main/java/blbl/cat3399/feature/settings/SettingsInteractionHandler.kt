...
class SettingsInteractionHandler(
    ...
    private fun exportStartToast(mode: AppConfigBackup.ExportMode): String =
        if (mode ==
            AppConfigBackup.ExportMode.CONFIG_WITH_CREDENTIALS
        ) {
            "正在导出配置与登录状态…"
        } else {
            "正在导出配置…"
    ...
    private fun showUploadLogsSuccessPopup(fileName: String) {
    ...
    private fun throwableMessage(t: Throwable): String = t.message?.takeIf { it.isNotBlank() } ?: "未知错误"
                    ...
                    val recycler =
                        (
                            LayoutInflater
                                .from(
                                    dialogContext,
                                ).inflate(blbl.cat3399.R.layout.view_popup_choice_list, null, false) as RecyclerView
                        ).apply {
    ...
    private fun showCastManagementDialog() {
        ...
        // 搜索设备
        actions.add(
            blbl.cat3399.core.ui.popup.PopupAction(
                ...
                kotlinx.coroutines.CoroutineScope(kotlinx.coroutines.Dispatchers.Main).launch {
                    try {
                        val devices =
                            blbl.cat3399.feature.cast.DlnaHelper
                                .discoverDevices(searchTimeout)
                        // 关闭搜索对话框
                        android.app.AlertDialog
                            .Builder(activity)
                            .create()
            ...
            .setItems(names) { _, which ->
                ...
                kotlinx.coroutines.CoroutineScope(kotlinx.coroutines.Dispatchers.Main).launch {
                    try {
                        ...
                        blbl.cat3399.feature.cast.DlnaHelper
                            .setAVTransportURI(device, url, title)
                        blbl.cat3399.feature.cast.DlnaHelper
                            .play(device)
                        // 保存上次设备
                        activity
                            .getSharedPreferences("cast_settings", android.content.Context.MODE_PRIVATE)
                            .edit()
                ...
                try {
                    val exportData =
                        buildString {
                            ...
                            appendLine(
                                "导出时间: ${java.text.SimpleDateFormat(
                                    "yyyy-MM-dd HH:mm:ss",
                    ...
                    .setPositiveButton("清理") { _, _ ->
                        try {
                            ...
                            activity.cacheDir?.listFiles()?.forEach {
                                if (it.isDirectory &&
                                    (
                                        it.name.contains("image") ||
                                            it.name.contains("picasso") ||
                                            it.name.contains("glide") ||
    ...
    private fun showVideoQualityProfileDialog() {
        ...
        fun qualityLabel(qn: Int): String =
            when (qn) {
                16 -> "360P"
                32 -> "480P"
                64 -> "720P"
    ...
    private fun showSkipSecondsDialog(
        title: String,
        current: Int,
        onSet: (Int) -> Unit,
    ) {
    ...
    private fun showDanmakuTemplateDialog() {
        val prefs = activity.getSharedPreferences("blbl_prefs", android.content.Context.MODE_PRIVATE)
        val templates =
            prefs.getStringSet("danmaku_templates", mutableSetOf())?.toMutableSet()
        ...
        android.app.AlertDialog
            ...
            .setPositiveButton("添加") { _, _ ->
                ...
                if (text.isNotEmpty()) {
                    templates.add(text)
                    activity
                        .getSharedPreferences("blbl_prefs", android.content.Context.MODE_PRIVATE)
                        .edit()
                        .putStringSet("danmaku_templates", templates)
                        .apply()
    ...
    private fun showVideoStatisticsShowDialog() {
        ...
        val prefsKeys =
            arrayOf("stats_show_view", "stats_show_danmaku", "stats_show_like", "stats_show_coin", "stats_show_share", "stats_show_fav")
    ...
    private fun showGestureSensitivitySlider(
        title: String,
        key: String,
        current: Int,
    ) {
    ...
    private fun showSkipSecondsInput(
        title: String,
        key: String,
        current: Int,
    ) {
