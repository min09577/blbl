...
Updated for the restored file structure with original properties."""
...
def get_positions():
    """Find insertion points in AppPrefs.kt"""
    ...
    # Find last KEY constant (before companion object closing)
    last_key = 0
    for i, line in enumerate(lines):
        if 'private const val KEY_' in line:
            last_key = i
    
    return last_v30, last_key
...
def add_feature(ver, idx, name, ftype, cn_desc):
    ...
    # Find last vN or v(N-1) property setter for this version
    insert_after = 0
    for i, line in enumerate(lines):
        if f'var v{ver}' in line or (ver > 31 and f'var v{ver-1}' in line):
            for j in range(i, min(i+5, len(lines))):
                if '.apply()' in lines[j]:
                    insert_after = j
                    break
    ...
    # Find last KEY constant and insert new key
    last_key = 0
    for i, line in enumerate(lines):
        if 'private const val KEY_' in line:
            last_key = i
...
for ver in range(31, 41):
    ...
    # Find last property of this version
    last_prop = 0
    for i, line in enumerate(lines):
        if ver > 31 and f'var v{ver-1}' in line:
            for j in range(i, min(i+5, len(lines))):
                if '.apply()' in lines[j]:
                    last_prop = j
                    break
