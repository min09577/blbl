...
def add_feature_to_appprefs(ver, idx, name, ftype, cn_desc):
    """Add a property and KEY to AppPrefs.kt"""
    filepath = os_chdir + "\\app\\src\\main\\java\\blbl\\cat3399\\core\\prefs\\AppPrefs.kt"
    ...
    # 1. Property: insert after last property of this version, or after last property of previous version
    insert_prop_after = None
    for i, l in enumerate(lines):
        if f'var v{ver-1}' in l or (f'var v{ver}' in l and name > l.split('var ')[1].split(':')[0] if 'var ' in l else False):
            # Find the setter line
            for j in range(i, min(i+5, len(lines))):
                if '.apply()' in lines[j]:
                    insert_prop_after = j
                    break
    
    if insert_prop_after is None:
        # Find last property setter overall
        for i in range(len(lines)-1, -1, -1):
            if '.apply()' in lines[i] and 'var v' in ''.join(lines[max(0,i-5):i]):
                insert_prop_after = i
                break
    
    # 2. KEY: insert after last KEY of this version, or after last KEY of previous version
    insert_key_after = None
    for i, l in enumerate(lines):
        if f'KEY_V{ver}_' in l:
            insert_key_after = i
    
    if insert_key_after is None:
        # Find last KEY overall
        for i in range(len(lines)-1, -1, -1):
            if 'private const val KEY_V' in lines[i]:
                insert_key_after = i
                break
    
    # Insert property block
    lines.insert(insert_prop_after + 1, prop_block)
    
    # Adjust key insertion point (shifted by property insertion)
    if insert_key_after >= insert_prop_after:
        insert_key_after += prop_block.count('\n')
    
    # Insert KEY
    lines.insert(insert_key_after + 1, key_line)
    
    with open(filepath, 'w', encoding='utf-8') as f:
        f.writelines(lines)
    
    return prop_name, key_name

def add_feature_to_playersettings(ver, idx, name, ftype, cn_desc):
    """Add a dialog function to PlayerSettings.kt"""
    filepath = os_chdir + "\\app\\src\\main\\java\\blbl\\cat3399\\feature\\player\\PlayerSettings.kt"
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()
    ...
    if ftype == "Boolean":
        func = (
            f"\n// {ver}.{idx}: {cn_desc}\n"
            f"internal fun PlayerActivity.show{prop_name}Toggle() {{\n"
            f"    val current = BiliClient.prefs.{prop_name}\n"
            f"    AppPopup.Builder(this)\n"
            f"        .title(\"{cn_desc}\")\n"
            f"        .option(switchText(current)) {{ BiliClient.prefs.{prop_name} = !current }}\n"
            f"        .show()\n"
            f"}}\n"
        )
    else:  # Int
        func = (
            f"\n// {ver}.{idx}: {cn_desc}\n"
            f"internal fun PlayerActivity.show{prop_name}Dialog() {{\n"
            f"    val current = BiliClient.prefs.{prop_name}\n"
            f"    AppPopup.Builder(this)\n"
            f"        .title(\"{cn_desc}\")\n"
            f"        .option(\"0\") {{ BiliClient.prefs.{prop_name} = 0 }}\n"
            f"        .option(\"1\") {{ BiliClient.prefs.{prop_name} = 1 }}\n"
            f"        .option(\"2\") {{ BiliClient.prefs.{prop_name} = 2 }}\n"
            f"        .option(\"3\") {{ BiliClient.prefs.{prop_name} = 3 }}\n"
            f"        .option(\"5\") {{ BiliClient.prefs.{prop_name} = 5 }}\n"
            f"        .option(\"10\") {{ BiliClient.prefs.{prop_name} = 10 }}\n"
            f"        .show()\n"
            f"}}\n"
        )
    
    # Append before the last closing brace or at the end
    # Find last internal fun
    last_func = content.rfind('internal fun')
    if last_func > 0:
        # Find the end of this function
        brace_pos = content.find('{', last_func)
        brace_count = 1
        pos = brace_pos + 1
...
for ver in range(31, 41):
    ...
    # Add section header for this version in AppPrefs.kt
    filepath = os_chdir + "\\app\\src\\main\\java\\blbl\\cat3399\\core\\prefs\\AppPrefs.kt"
    with open(filepath, 'r', encoding='utf-8') as f:
        lines = f.readlines()
    
    # Find last property setter
    last_setter = 0
    for i in range(len(lines)-1, -1, -1):
        if '.apply()' in lines[i]:
    ...
    section_header = f"\n    // ===== v{ver}.x =====\n"
    lines.insert(last_setter + 1, section_header)
    with open(filepath, 'w', encoding='utf-8') as f:
        f.writelines(lines)
    
    # Also add KEY section header
    with open(filepath, 'r', encoding='utf-8') as f:
        lines = f.readlines()
    ...
    key_header = f"\n        // ===== v{ver}.x KEY =====\n"
    lines.insert(last_key + 1, key_header)
    with open(filepath, 'w', encoding='utf-8') as f:
        f.writelines(lines)
    ...
    for idx, (name, ftype, cn_desc, en_desc) in enumerate(features, 1):
        ...
        add_feature_to_appprefs(ver, idx, name, ftype, cn_desc)
        add_feature_to_playersettings(ver, idx, name, ftype, cn_desc)
...
print(f"\nDone! Total commits: {commit_count}")
